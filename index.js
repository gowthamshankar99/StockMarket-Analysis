        ...
        request('https://iyz9xlw3ba.execute-api.us-east-1.amazonaws.com/Prod/getIndividualStockTickers')
            .then(function (res) {
                ...
                for (let item of JSON.parse(res).Items) {
                    ...
                    request('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=' + item.tickerName.S + '&apikey='+rand)
                        .then(function (done) {
                            //console.log(done);
                            if(JSON.parse(done)['Time Series (Daily)'] != undefined)
                            {
                                console.log(JSON.parse(done)['Time Series (Daily)']['2018-07-27']);
                                if (JSON.parse(done)['Time Series (Daily)'] != undefined) {
                                    tickerDetails = JSON.parse(done)['Time Series (Daily)']['2018-07-27'];
                                    tickerDetails.ticker = item.tickerName.S;
                                    tickerDetails.date = new Date();
                                    ...
                                    if (JSON.parse(res).Items.length == TickerNames.length) {
